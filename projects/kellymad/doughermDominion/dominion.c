...
int minionCard(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
    ...
    //discard card from hand
    //printf("Made it to before discard Minion\n");
    //printf("1 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
    discardCard(handPos, currentPlayer, state, 0);
    //printf("2 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
    if (choice1)
    {
        //printf("Inside choice1\n");
        state->coins = state->coins + 2;
    }
    else if (choice2)       //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        //printf("Inside choice2\n");
        //discard hand
        //printf("3 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
        while(numHandCards(state) > 1)
        {
            //printf("Inside while loop to discard p hand\n");
            discardCard(handPos, currentPlayer, state, 0);
            //printf("4 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
        }

        //draw 4
        for (i = 0; i < 4; i++)
        {
            //printf("inside loop to draw 4\n");
            drawCard(currentPlayer, state);
            //printf("5 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
        }

        //other players discard hand and redraw if hand size > 4
        for (i = 0; i <=state->numPlayers; i++)
        {
          //printf("int i = %d, int numPlayers = %d\n", i, state->numPlayers);
          //printf("inside loop of players\n");
            if (i != currentPlayer)
            {
              //printf("if not cp\n");
                if ( state->handCount[i] > 4 )
                {
                  //printf("if handcount > 4\n");
                    //discard hand
                    while( state->handCount[i] > 0 )
                    {
                      //printf("inside while loop to discard np player's hand\n");
                        discardCard(handPos, i, state, 0);
                        //printf("6 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
                      //printf("made it after discardCard\n");
                    }
                    //draw 4
                    for (j = 0; j < 4; j++)
                    {
                      //printf("inside loop to draw 4 for np\n");
                        drawCard(i, state);
                        //printf("7 state->handCount[currentPlayer] = %d\n", state->handCount[currentPlayer]);
                    }
                }
            }
        }
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag)
{
    ...
    //printf("Made it out of if trashflag\n");
    ...
    {
      //printf("If handpos == state->handCount[currentPlayer]-1\n");
        //reduce number of cards in hand
        state->handCount[currentPlayer]--;
      //  printf("4 handCount %d\n", state->handCount[currentPlayer]);
    }
    else if ( state->handCount[currentPlayer] == 1 ) //only one card in hand
    {
      //printf("If state->handCount[currentPlayer] == 1\n");
        //reduce number of cards in hand
        state->handCount[currentPlayer]--;
        //printf("3 handCount %d\n", state->handCount[currentPlayer]);
    }
    else
    {
        //printf("else...\n");
        //replace discarded card with last card in hand
        //printf("2 handCount %d\n", state->handCount[currentPlayer]);
        //printf("state->hand[currentPlayer][ (state->handCount[currentPlayer] - 1)] = %d\n", state->hand[currentPlayer][ (state->handCount[currentPlayer] - 1)]);
    ...
    //printf("Made it to end of discard function\n");
    return 0;
}
