...
void posCase1(int choice1, int choice2, struct gameState *post, int handPos, int currentPlayer) {
...
//----------------------------------------------------------------------------------
    //run minionEffect()
    r = minionCard(choice1, choice2, post, handPos, currentPlayer);
...
void posCase2(int choice1, int choice2, struct gameState *post, int handPos, int currentPlayer) {
    ...
    //----------------------------------------------------------------------------------
        ...
        for(int j = 0; j<2; j++)
        {
          ...
          for(int m = 0; m<4; m++)
          {
            ...
            pre.hand[currentPlayer+j][m] = pre.deck[currentPlayer+j][(pre.deckCount[currentPlayer+j] - 1)];
            pre.handCount[currentPlayer+j]++;
...
void posCase3(int choice1, int choice2, struct gameState *post, int handPos, int currentPlayer) {
...
void negCase(int choice1, int choice2, struct gameState *post, int handPos, int currentPlayer) {
    //set initial conditions
    //------------------------------------------------------------------------------
        post->hand[currentPlayer][0] = minion;
        post->hand[currentPlayer][1] = minion;
...
int main() {
  ...
  posCase1(1, 0, &G1, handPos, currentPlayer);
  posCase2(0, 1, &G2, handPos,currentPlayer);
  posCase3(0, 1, &G3, handPos,currentPlayer);
  printf("Starting negCase1: choice1 = 2 = invalid entry - expect error\n");
  negCase(2, 0, &G4, handPos, currentPlayer);
  printf("End negCase1.\n");
  printf("Starting negCase2: choice2 = 2 = invalid entry - expect error\n");
  negCase(0, 2, &G5, handPos, currentPlayer);
  printf("End negCase2.\n");
  printf("Starting negCase3: choice1 = choice2 = 0 = invalid entry - expect error\n");
  negCase(0, 0, &G6, handPos, currentPlayer);
  printf("End negCase3.\n");
  printf("Ending unittest2 for minionCard().\n");
